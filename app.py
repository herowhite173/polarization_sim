...
# ========== 新增缓存：主光路绘图（matplotlib 对象用 cache_resource） ==========
@st.cache_resource
...
# ========== 新增缓存：马吕斯曲线绘图 ==========
@st.cache_resource
def plot_malus_scatter(current_angle, bg, collected_data):
    import random
...
@media (min-width: 769px) {
    ...
    if mobile_view:
        ...
        if st.session_state.mode == "sim":
            fig_scat, new_data = plot_malus_scatter(st.session_state.analyzer_angle,
