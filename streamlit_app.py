...
# 绘图缓存：增加 _cache_force 忽略参数，强制角度变化刷新缓存
...
@media (min-width: 769px) {
    ...
    if mobile_view:
        ...
        if st.session_state.mode == "demo":
            ...
            with col_input:
                st.session_state.polarizer_angle = st.number_input(
                    "起偏器角度", 0, 360,
        ...
        if st.session_state.mode == "sim":
            ...
            with col_input:
                st.session_state.bg_noise = st.number_input(
        ...
        with col_input:
            if st.session_state.mode == "demo":
                st.session_state.analyzer_angle = st.number_input(
                    "检偏器角度", 0, 360, st.session_state.analyzer_angle,
    ...
    # 电脑端布局
        ...
        with col_control:
            ...
            with c4:
                if st.session_state.mode == "demo":
                    st.session_state.analyzer_angle = st.number_input(
                        "检偏器", 0, 360, st.session_state.analyzer_angle,
                        10, "%d", label_visibility="collapsed",
                        key=f"a{st.session_state.reset_counter}"
                    )
                else:
